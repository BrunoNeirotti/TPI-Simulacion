...
\begin{enumerate}
  \item Despacho desde una cabecera, con intervalo tomado de la distribución
    ajustada sobre los intervalos medidos (líneas actuales) o de la variable de
    escenario de intervalo entre trenes (Línea F, ver sección~\ref{sec:variables}).
...
\begin{itemize}
  ...
  \item Inclusión o no de las etapas marcadas como incompletas (4,1~\% de la
    demanda).
\end{itemize}
...
\begin{itemize}
  ...
  \item En el escenario futuro, la Línea F se \textbf{agrega} como nodos y aristas
    ...
    supuesto impuesto línea por línea.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Distribuciones de intervalos de despacho}, a ajustar sobre los
    663.709 intervalos medidos, siguiendo el procedimiento de histogramas, máxima
    verosimilitud y bondad de ajuste visto en la materia.
...
\begin{itemize}
  ...
  \item \textbf{Detención de las líneas actuales}: endógena (según ascensos y
    descensos), con piso de 24~s de diseño del GTFS.
  \item \textbf{Comportamiento del pasajero que no aborda}: espera la próxima
    ...
    abandona el sistema. Regla declarada, no medida --- falta que el grupo la
    confirme.
  ...
  \item \textbf{Intervalo entre trenes de la Línea F como variable de escenario, no
    ...
    no existe.
  \item \textbf{Demanda futura de la Línea F}: no hay medición posible; el perfil de
    SBASE de 2019 se usa como contraste declarado, con la salvedad de que implica
    una concentración horaria que la red actual no exhibe.
\end{itemize}
...
\begin{enumerate}
  ...
  \item ¿Qué tan sensibles son los indicadores centrales a los supuestos propios del
    trabajo: la desagregación temporal fuera de las horas pico, el tratamiento de
    las etapas incompletas, y la regla de comportamiento del pasajero que no
    aborda?
\end{enumerate}
